...
- 思路：使用两个队列。树的层次遍历，使用队列。
    - 使用两个队列，和两个数组。第一个队列出来后，下一层的数据按照从左到右的顺序防毒第二个队列，先进先出，出来的顺序也是从左到右。
...
- 思路：不需要使用两个队列来存储当前层的节点和下一层的节点，因为在开始遍历第一层的节点时，当前队列中的节点数就是当前层的节点数，就能保住这次遍历的都是当前层的节点。 
    - 使用一个队列和一个数组。
...
15. [二叉树中和为某一值的路径](https://github.com/JINGbw/NewCoder/blob/master/%E6%A0%91/%E4%BA%8C%E5%8F%89%E6%A0%91%E4%B8%AD%E5%92%8C%E4%B8%BA%E6%9F%90%E4%B8%80%E5%80%BC%E7%9A%84%E8%B7%AF%E5%BE%84.cpp)
- 题目：输入一颗二叉树的根节点和一个整数，打印出二叉树中结点值的和为输入整数的所有路径。路径定义为从树的根结点开始往下一直到叶结点所经过的结点形成一条路径。(注意: 在返回值的list中，数组长度大的数组靠前)
- 思路：深度优先遍历，根节点，左子节点，右子节点（只能是先序遍历）
    - 递归函数：计算当前子树，是否有和为exceptnumber的路径，当走到叶节点将值压入res。
